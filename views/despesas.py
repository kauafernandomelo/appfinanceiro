...
class DespesasView(BaseView):
    ...
    def _criar_header(self):
        ...
        self._criar_titulo(header, "Despesas").pack(side="left")
    ...
    def _criar_formulario(self):
        ...
        self.entry_desc = self._criar_entry(g, "Ex: Aluguel, Supermercado...")
        ...
        parcelas_frame.grid(row=0, column=4, rowspan=2, sticky="ew", padx=(6, 0))
        parcelas_frame.grid_columnconfigure(1, weight=1)

        self.chk_parcelado = ctk.CTkCheckBox(
            parcelas_frame, text="Parcelar?", font=ctk.CTkFont(size=11),
        ...
        self.chk_parcelado.grid(row=0, column=0, sticky="w", padx=(0, 4))
    ...
    def _criar_search(self):
        ...
        self.entry_search = self._criar_entry(sg, "Buscar despesa...")
    ...
    def _salvar(self):
        ...
        try:
            with get_connection() as conn:
                ...
                if self._edit_id:
                    conn.execute("UPDATE despesas SET descricao=?,valor=?,data=?,categoria_id=? WHERE id=?",
                                 (desc, val, data, cid, self._edit_id))
                    mostrar_toast(self, f"Despesa '{desc}' atualizada!")
                    self._edit_id = None
                    self.btn_add.configure(text="+ Adicionar", fg_color=self.colors.get("red", "#d63031"))
                elif parcelado:
    ...
    def _editar(self, did):
        try:
            with get_connection() as conn:
                r = conn.execute("SELECT * FROM despesas WHERE id=?", (did,)).fetchone()
                if not r:
                    return
                cat_name = None
                if r["categoria_id"]:
            ...
            mostrar_toast(self, "Erro ao carregar despesa: " + str(e), "erro")

    def _excluir(self, did):
        if self._confirmar_exclusao("Excluir Despesa", "Deseja excluir esta despesa?"):
            try:
                with get_connection() as conn:
        ...
        try:
            with get_connection() as conn:
                self._all_rows = conn.execute(
                    """SELECT d.id, d.descricao, d.valor, d.data, COALESCE(c.nome,'Sem categoria') as cat
    ...
    def _renderizar(self, rows, total, total_pages):
        ...
        for i, r in enumerate(page_rows):
            ...
            btn_edit = ctk.CTkButton(row, text="\u270e", width=28, height=28, corner_radius=6,
                           ...
                           command=lambda did=r["id"]: self._editar(did))
            ...
            btn_del = ctk.CTkButton(row, text="\u2715", width=28, height=28, corner_radius=6,
                           font=ctk.CTkFont(size=13, weight="bold"),
                           fg_color=self.colors.get("red", "#d63031"), hover_color="#c0392b",
                           command=lambda did=r["id"]: self._excluir(did))
